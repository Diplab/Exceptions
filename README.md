...
例外處理的本意是，在程式錯誤發生時，能夠有明確的方式通知API客戶端，讓客戶端採取進一步的動作修正錯誤，Java是唯一採用Checked exception的語言，這有兩個目的：一是文件化，受檢例外宣告會是API操作介面的一部份，客戶端只要查閱文件，就可以知道方法可能會引發哪些例外，並事先加以處理，而這是API設計者決定是否拋出Checked exception時的考量之一，另一個目的是提供編譯器資訊，
讓編譯器能夠在編譯時期就檢查出API客戶端沒有處理例外。

問題在於有些錯誤發生而引發例外時，你是無力回復的，例如SQLException是受檢例外，而如果例外的發生原因是資料庫連線異常，而連線異常的原因是由於實體線路問題，那麼無論如何你都不可能使用try、catch回復到正常可運作的情況。
QLException發生時，最好的方法是將例外浮現至呈現層，例如網頁技術，將錯誤訊息於網頁上顯示出來給管理人員，為了讓例外往上浮現，你也許會選擇在每個方法呼叫上都宣告throws SQLException，但假設，這個方法的呼叫是在整個應用程式的底層，
這樣的作法也許會造成許多程式碼的修改

Checked exception的本意是好的，有助於程式設計人員注意到例外的可能性並加以處理，但在應用程式規模增大時，會對逐漸對維護造成困難
